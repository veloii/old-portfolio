...
export default function UnifiedBookmarks() {
  const { setUnifiedBookmarks } = useContext(CurrentProjectContext);
  const ref = useRef<HTMLDivElement | null>(null);
  const isVisible = useOnScreen(ref);

  useEffect(() => {
    setUnifiedBookmarks(isVisible);
  }, [isVisible]);
  ...
  return (
    <motion.div
      ...
      className="pb-64"
    >
      <div className="pl-16 max-w-7xl mx-auto bg-gray-800 bg-opacity-10 rounded-3xl border shadow-lg px-6 border-gray-800">
            ...
            <ul className="space-y-1 pt-4 ">
              <li className="text-lg max-w-3xl leading-relaxed text-gray-300">
                ...
                <a
                  ...
                  target="_blank"
                  className="font-bold text-violet-300 hover:text-violet-400 transition ease-in-out duration-200"
                  rel="noreferrer"
                >
                  extension
                </a>
